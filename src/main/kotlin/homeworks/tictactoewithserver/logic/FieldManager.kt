...
class FieldManager(private val rawField: List<Int>, private val linearSize: Int) {

    val cells = rawField.withIndex().map { Cell(it.index, it.value) }
    ...
    private fun getColumns(): List<Line> {
        return IntRange(0, linearSize - 1).map { i -> Line(cells.slice(i until rawField.size step linearSize)) }
    }
